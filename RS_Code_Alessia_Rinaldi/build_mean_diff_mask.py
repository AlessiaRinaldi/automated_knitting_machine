...
def preprocess(bgr_or_gray, use_clahe=True):
    ...
    if bgr_or_gray.ndim == 3:
        ...
        g = bgr_or_gray.copy()
    # riduzione rumore
    g = cv2.GaussianBlur(g, (3,3), 0)  #g:input img; (3,3): kernel dim (3 pixel x 3 pixel); 0: Gaussian standard dev. (lungo X)[se = 0, opencv calcola automaticamente]
    # equalizzazione locale per preservare ombre/texture
    if use_clahe:
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(4,4)) # 2.0 perchè si amplifica rumore con 2.0 aumenti contrasto ed eviti rumore;(4,4)->opencv divide l’immagine in 4 blocchi in orizzontale e 4 in verticale (16 tot)
        g = clahe.apply(g)
    return g
...
def load_stack(folder, target_hw=(H_IMG, W_IMG)):
    ...
    if not imgs:
        raise RuntimeError(f"no valid img in {folder}")
    stack = np.stack(imgs, axis=0)  # [N, H, W]
    return stack, paths

def bottom_roi_mask(h=H_IMG, w=W_IMG, roi_h=ROI_H):
    #serve fare ROI nella ROI perchè parte alta non mi interessa, valutare se fare crop diverso fin da subito
...
def build_mask(pos_stack, neg_stack, bilateral=True):
    ...
    #fisher-like score
    eps = 1e-6
    S = np.abs(mu_pos - mu_neg) / (std_pos + std_neg + eps) #S alto -> pixel molto discriminante; S basso -> pixel poco utile

    #serve a portare il min a 0 e max a 1
    S -= S.min()
    if S.max() > 0:
        S /= S.max()

    #tieni solo il rettangolo in basso 55x76, sopra nero
    ...
    # Hard mask via Otsu (solo nella ROI, fuori ROI 0) NON UTILIZZATA!!!
    S8 = (S*255.0).astype(np.uint8)
...
def main():
    ...
    print(f"[OK] Soft mask saved in: {soft_path}")
    print(f"[OK] Hard mask (Otsu) saved in: {hard_path} (threshold Otsu={thr:.1f})")
    ...
    print(f"Debug LLR-like: NCC_pos={ncc_pos:.3f}, NCC_neg={ncc_neg:.3f}, diff={llr_like:.3f}")
